...
String questionApi = Platform.isAndroid
    ? 'http://10.0.2.2:8000/answer/'
    : 'http://127.0.0.1:8000/answer/';

class QuestionApiClient {
  final http.Client httpClient;
  QuestionApiClient({required this.httpClient});

  dynamic questionFetchData() async {
    try {
      http.Response response = await http.get(Uri.parse(questionApi));
